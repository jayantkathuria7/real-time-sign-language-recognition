...
# Add this to check for deployment environment
...
if is_deployed:
    # For deployed environment, use file upload instead
    st.sidebar.markdown("### Camera Access")
    st.sidebar.info("When deployed, direct camera access is not available. Please upload a video file instead.")
    uploaded_file = st.sidebar.file_uploader("Upload a video for sign recognition", type=["mp4", "avi", "mov"])
    camera_id = None
else:
    # Local development environment with direct camera access
...
# Control buttons
if is_deployed and uploaded_file is None:
    st.info("Please upload a video file from the sidebar to begin sign language recognition.")
    start_button = st.button("Process Video", disabled=True)
    stop_button = st.button("Stop Processing", disabled=True)
else:
    start_button = st.button("Start Camera" if not is_deployed else "Process Video")
    stop_button = st.button("Stop Camera" if not is_deployed else "Stop Processing")
...
if st.session_state.run_camera and model_loaded:
    if is_deployed and uploaded_file is not None:
        # Process uploaded video file
        status_text.info("Processing uploaded video... Please wait.")
        ...
        try:
            ...
            if not cap.isOpened():
                ...
                status_text.success("Video loaded successfully!")

                # Continue with video processing (same as camera processing code)
                ...
                while st.session_state.run_camera:
                    ret, frame = cap.read()
                    if not ret:
                        status_text.info("End of video reached.")
                        break
                    ...
                    if len(sequence) == SEQUENCE_LENGTH:
                        ...
                        if confidence > confidence_threshold:
                            ...
                            cv2.putText(frame, f"Sign: {pred_label}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                        (0, 255, 0), 2)
                            cv2.putText(frame, f"Conf: {confidence:.2f}", (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                                        (0, 255, 0), 2)
                            ...
                            if pred_label != last_prediction and translation_module_available:
                                ...
                                if translated_text:
                                    translation_placeholder.markdown(
                                        f"**Translation ({target_language}):** {translated_text}")
                    ...
                    time.sleep(0.01)  # You might need to adjust this for video playback
            ...
            status_text.error(f"Error processing video: {e}")
            st.session_state.run_camera = False

    else:
        # Local camera access code (your original code)
        ...
        try:
            ...
            if not cap.isOpened():
                ...
                # Your existing camera processing code continues here...
                sequence = deque(maxlen=SEQUENCE_LENGTH)
                last_prediction = ""
...
if not st.session_state.run_camera:
    if is_deployed:
        video_placeholder.info("Upload a video file and click 'Process Video' to begin sign language recognition.")
    else:
    ...
    if is_deployed:
        st.markdown("""
        1. Upload a video file from the sidebar
        2. Click the 'Process Video' button
        3. The app will recognize, translate, and speak the signs in the video
